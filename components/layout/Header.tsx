...
import { Avatar, Badge, Button, CounterBadge, Divider, Drawer, DrawerBody, DrawerFooter, DrawerHeader, DrawerHeaderTitle, Menu, MenuItem, MenuList, MenuPopover, MenuTrigger, Tooltip, makeStyles, tokens } from '@fluentui/react-components';
import { AlertUrgent24Regular, ArrowUpload24Regular, CalendarAdd24Regular, Navigation24Regular, SignOut24Regular } from '@fluentui/react-icons';
...
import { getNotifications, markNotificationAsRead, markNotificationAsUnread } from '@/services/api/notification';
...
export default function Header() {
    ...
    const {
        ...
        queryFn: () => getNotifications({ limit: 20, order_by: 'created_at', dir: 'desc' }).then(res => res.data),
    ...
    const toggleReadMutation = useMutation({
        ...
        onSuccess: (data, { isRead }) => {
            showToast(isRead ? 'info' : 'success', isRead ? t('markedAsUnread') : t('markedAsRead'));
        },
    });
    ...
    return (
        <>
                        ...
                        <MenuPopover>
                            <MenuList>
                                <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    ...
                                    <div style={{ fontSize: '12px', color: 'var(--colorNeutralForeground3)' }}>{user?.email}</div>
                                    {user?.position && (
                                        <div style={{ fontSize: '12px', color: 'var(--colorNeutralForeground3)' }}>{user.position}</div>
                                    )}
                                </div>
                                <Divider />
            ...
            <Drawer open={isNotiOpen} position="end" onOpenChange={(_, data) => setIsNotiOpen(!!data.open)}>
                ...
                <DrawerBody>
                    <div className={styles.drawerList}>
                        {notificationsLoading ? (
                            ...
                            <div style={{ textAlign: 'center', padding: '20px', color: 'var(--colorStatusDangerForeground1)' }}>
                                ...
                                return (
                                    ...
                                    >
                                        <div className={styles.drawerItemHeader}>
                                            ...
                                            {!n.is_read && <Badge appearance="filled" color="brand" size="small">New</Badge>}
                                        </div>
                                        <div className={styles.drawerItemSub}>{displayInfo.message}</div>
                                        <div style={{ fontSize: '10px', color: 'var(--colorNeutralForeground3)', marginTop: '4px' }}>
                ...
                <DrawerFooter>
                    <Button appearance="secondary" onClick={() => setIsNotiOpen(false)}>{t('close')}</Button>
                </DrawerFooter>
            </Drawer>
